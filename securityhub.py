    ...
    def get_findings(self, sh_filters):
        ...
        while True:
            try:
                ...
                self.logger.error("An error occurred when attempting to gather SecurityHub data - %s", err)
            ...
            if "Findings" not in list(response.keys()):
                self.logger.error("get_findings returned unexpected data (No 'Findings' key) - %s", str(response))
                exit(1)
