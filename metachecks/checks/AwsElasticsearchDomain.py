...
class Metacheck:
    ...
    def _describe_elasticsearch_domain(self):
        try:
            response = self.client.describe_elasticsearch_domain(DomainName=self.resource_id)
        except ClientError as err:
            if err.response['Error']['Code'] in ["AccessDenied", "UnauthorizedOperation"]:
                self.logger.error("Access denied for describe_elasticsearch_domain: " + self.resource_id)
                return False
            else:
                self.logger.error("Failed to describe_elasticsearch_domain: " + self.resource_id)
                return False
    ...
    def it_has_public_endpoint(self):
        ...
        if public_endpoints: return public_endpoints
        return False
    ...
    def it_has_access_policies(self):
        ...
        if self.elasticsearch_domain:
            if 'AccessPolicies' in self.elasticsearch_domain:
                access_policies = json.loads(self.elasticsearch_domain['AccessPolicies'])['Statement']
        if access_policies: return access_policies
        return False
