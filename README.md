...
**MetaHub** queries the affected resources in the affected account to add extra information using **MetaChecks** (`--meta-checks`) and **MetaTags** (`--meta-tags`) . **MetaChecks** are custom python checks that you can run on the affected resources to increase the level of confidence in your findings, like if a resource is public, if it's has a policy, is attached to a public resource and more, and **MetaTags** let you enrich your findings with the tags that are associated with the resource. You can then query the results of these outputs, for example, listing all resources that are effectively public and are tagged with a tag `Environment=production`. See [MetaChecks](#MetaChecks) and [MetaTags](#MetaTags).
